...
can be truthfully implemented in a direct revelation mechanism. For ex
post equilibrium, it is also true as proved in Appendix
\ref{Appendix_A}. Actually, Revelation Principle has not told all that
has been shown in the proof. In the proof, we establish a direct
revelation mechanism that for each strategy profile in the original
mechanism, there is a strategy profile in the direct revelation
mechanism that has the same result:
$g(s_1,\cdots,s_n)=h(\sigma^1(s_1),\cdots,\sigma^n(s_n))\in
F(s_1,\cdots,s_n)$. Then we showed that an ex post equilibrium result
in the original mechanism is also an ex post equilibrium result in
this direct revelation mechanism.  It is called the equivalent
direct mechanism of the original mechanism.
%In fact, the equivalent direct mechanism is equivalent to the original mechanism not only in ex post equilibrium results.
Remember that we have
described an assets_inheritance problem in the examples section
\ref{assets_inheritance} in the Introduction Chapter. The second
mechanism is the equivalent direct mechanism of the first mechanism.
%For designing new
%mechanisms, it seems that considering only strategyproof direct mechanism is
%enough.

Therefore when studying ex post equilibrium results for existing
mechanisms which are not direct revelation mechanism, we can use a
equivalent direct revelation mechanism with the same ex post
equilibrium results for expositional convenience\footnote{We put the
...
manipulation which means a best report choice given others' report is
frequently used in later analysis of the strategical interaction of
students in the Chinese college admission mechanisms. \footnote{ It is
...
Now, let's introduce three important class of mechanisms: DA mechanism, Boston
mechanism and parallel mechanisms. DA mechanism is what we have investigated in the previous section, but it has never been adopted in China. Boston mechanism or the Immediate Acceptance (IA)
mechanism  had been the only mechanism used in Chinese
...
manipulation is often more important than your score level. The
problem arise from the special priority structure in the
mechanism. Priority of a student in a college is based on your
reported preference list firstly and based on your score secondly, a
...
Each student reports his or her preference list, each college report its priority criterion(usually score plus some tie-breaking rules),then after collecting all these preference list and priority criterion, the social planner runs the DA procedure
to produce a final matching result.
\end{definition*}

\begin{definition*}direct revelation IA mechanism: Each students
report his or her preference list, each college reports its priority
criterion(usually score plus some tie-breaking rules),then after
collecting all these preference list and priority criterion, the
...
\begin{definition*} characteristic list of a parallel mechanism:
  An infinite list of integers greater than zero $(n_1,n_2,\cdots)$ is the characteristic list of a parallel mechanism if the $n_1,n_2,\cdots$
  define the choice-band size for the 1st ranked, 2nd ranked, \cdots  choice-band.
\end{definition*}

\begin{definition*}direct revelation parallel mechanisms: Each student
  report his or her preference list, each college report its priority
  criterion(usually score plus some tie-breaking rules),after
  collecting all these preference list and priority criterion, the
  social planner uses the preference list to produce a list of
  choice-bands according to the characteristic list $(n_1,n_2,\cdots)$
  of the parallel mechanism such that the 1st ranked choice-band
  includes the top $n_1$ colleges in the preference list, the 2nd
  ranked choice-band includes the top $n_2$ of the remaining colleges in the preference list, and so on. Then the social planner adjusts the
  priority rule such that the reported priority criterion of a college
...
list $(+\infty,0,0,\cdots)$ and $(1,1,1,\cdots)$. Compared with the parameter $e$ class of mechanisms in \parencite{YanChenJPE}, our formulation incorporates more mechanisms. Because we allow different ranked choice-band to be of different sizes by introducing a list of integers as characteristic parameters instead of just an integer as the parameter.
...
The formal proof is ommited. An example can be easily established to demonstrate it.
...
achieved with perfect manipulation under the IA mechanism, some
discussion can be found in \parencite{Ergin2006}; as to matching
results that can be achieved with perfect manipulation under the
parallel mechanism, relevant discussion can be found
in \parencite{YanChenJPE}.
...
manipulation under the IA and parallel mechanism  is the same with the one under  the DA mechanism, which is  unique , stable and constrained efficient. 
\end{thm}
...
As a direct result of the above propostion and the optimal stableness and non-manipulability of DA mechanism, we have the following corollary.
